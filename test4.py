...
def flatten(x):
    #把P保留第一个维度，把第一个维度包含的每一子张量展开成一个行向量，返回张量是一个二维的，shape=（batch_size，….）
    #例如CNN的conv层输出的tensor的shape为[batch_size, height, width, channel], 刚展开会就是[batch_size, height * width * channel]
...
def generator(z, train_sign=True, reuse=False, scope="generator"):
    with tf.variable_scope(scope, reuse=reuse):
        ch = 1024
        #x = lrelu(batch_norm_layer(linear(z, 224, name='l1'), train_sign), 0.02)
        #x = tf.reshape(linear(x, 7*2*16, name='l2'), [IC_datasets.batch_size, 7, 2, 16])     #[-1,7,2,16]
        #x = tf.reshape(z, [IC_datasets.batch_size, 7, 2, 16])
...
# def creat_fmimg(img_dir):
#     img_list, _, _ = get_list(img_dir)
#     added_img = np.ones((84,16),dtype='uint8')
#     for img_filename in img_list:
#         add_img = cv2.imread(img_filename, 0)
#         #print(src_img.shape)
#         added_img = cv2.add(added_img, add_img)
#     cv2.imwrite(os.path.join('./test_output', "fm.jpg"), added_img)
#     return added_img
...
def img_process(epoch, src_dir, gen_dir, fm_num):
    ...
    for src_filename, gen_filename, label, name in zip(src_list, gen_list, label_list, name_list):
        ...
        scrop_list.append(scrop)
        img_num = img_num + 1
        if img_num == fm_num:
            break
    max_scrop = np.max(scrop_list)
    for src_filename, gen_filename, label, name in zip(src_list, gen_list, label_list, name_list):
        src_img = cv2.imread(src_filename)
        gen_img = cv2.imread(gen_filename)
        ...
        if int(label) == 0 and img_label == 1:
...
def main():
    ...
    # sheet.write(0, 1, 'num_rignt')
    # sheet.write(0, 2, 'num_error')
    # sheet.write(0, 3, 'num_omiss')
    #
    # workbook.save(IC_datasets.excel_dir)
